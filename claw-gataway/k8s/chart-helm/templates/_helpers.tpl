...
requests:
  ...
  nvidia.com/gpu: {{ required "Value 'gpu.count' must be defined !" .Values.gpu.count | quote }}
  {{- end }}
limits:
...
{{- $storageType := "local" -}}
{{- if and .Values.extraInit .Values.extraInit.storage .Values.extraInit.storage.type -}}
{{- $storageType = .Values.extraInit.storage.type -}}
{{- end -}}
{{- if eq $storageType "local" -}}
hostPath:
  path: {{ required "Value 'extraInit.storage.hostPath' must be defined for local storage !" .Values.extraInit.storage.hostPath | quote }}
